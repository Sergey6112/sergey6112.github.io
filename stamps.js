...
const byImprint=document.querySelector("#stamp-by-imprint");
...
function update(){
  ...
  document.querySelector("#stamp-summary-text").textContent=`${product.value} · ${shape.value.toLowerCase()} форма · ${count()} шт.${byImprint.checked?" · по оттиску":""}${urgent.checked?" · срочное изготовление":""}`;
...
function message(){
  ...
  return["Здравствуйте! Хочу заказать печать или штамп в МОНОПРИНТ.","",`Изделие: ${product.value}`,`Форма: ${shape.value}`,`Печать по оттиску: ${byImprint.checked?"да":"нет"}`,`Количество: ${count()} шт.`,`Срочно: ${urgent.checked?"да":"нет"}`,`Файл: ${filename}`,"","Прошу рассчитать стоимость и срок изготовления.",filename!=="не выбран"?"Файл прикреплю следующим сообщением.":""].filter(Boolean).join("\n");
}
